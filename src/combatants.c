...
void CombatantDrawKnight(Vector2 pos, bool facingRight, CombatantPose pose)
{
    ...
    Color armor   = (Color){ 180, 190, 205, 255 };
    Color armorHi = (Color){ 220, 228, 240, 255 };
    Color armorLo = (Color){ 105, 115, 135, 255 };
    Color cape    = (Color){ 90, 20, 30, 255 };
    Color skin    = (Color){ 225, 185, 150, 255 };
    Color leather = (Color){ 90, 60, 40, 255 };
    Color blade   = (Color){ 230, 235, 245, 255 };
    Color shield  = (Color){ 60, 90, 150, 255 };
    Color gold    = (Color){ 220, 180, 70, 255 };
    ...
    for (int s = -1; s <= 1; s += 2)
    {
        Vector2 sh = Flip((Vector2){ 15 * s, -12 }, facingRight);
        DrawCircleV((Vector2){ px + sh.x, py + sh.y }, 8.0f, armor);
        DrawCircleLines((int)(px + sh.x), (int)(py + sh.y), 8.0f, armorLo);
    }

    /* Shield (rear arm) */
    {
        Vector2 sh = Flip((Vector2){ -14, -8 }, facingRight);
        float sx = px + sh.x, sy = py + sh.y;
    ...
    /* Sword arm + blade (front arm) */
    {
        Vector2 sh = Flip((Vector2){ 16, -10 }, facingRight);
        float sx = px + sh.x, sy = py + sh.y;
...
void CombatantDrawSlime(Vector2 pos, bool facingRight, CombatantPose pose)
{
    ...
    float w = 44.0f / squash;
    float h = 30.0f * squash;

    Color body   = (Color){  80, 200, 110, 235 };
    Color bodyHi = (Color){ 160, 240, 180, 200 };
    Color bodyLo = (Color){  40, 130,  70, 255 };
    Color core   = (Color){  60, 160,  90, 255 };

    DrawEllipse((int)pos.x, (int)(pos.y + h * 0.5f + 4), w * 0.6f, 6.0f,
                (Color){ 0, 0, 0, 80 });

    DrawEllipse((int)pos.x, (int)(pos.y + 2), w * 0.5f, h * 0.5f, body);
    DrawEllipse((int)(pos.x - w * 0.15f), (int)(pos.y - 4), w * 0.22f, h * 0.28f, bodyHi);

    DrawEllipse((int)pos.x, (int)(pos.y + h * 0.42f), w * 0.52f, 4.0f, bodyLo);
    ...
    DrawCircle((int)(pos.x - w * 0.2f), (int)(pos.y - h * 0.25f), 3.0f,
...
void CombatantDrawGoblin(Vector2 pos, bool facingRight, CombatantPose pose)
{
    ...
    Color skin   = (Color){ 110, 170,  90, 255 };
    Color skinLo = (Color){  70, 120,  60, 255 };
    Color rag    = (Color){ 130,  90,  50, 255 };
    Color ragLo  = (Color){  90,  60,  30, 255 };
    Color eye    = (Color){ 240, 220,  60, 255 };
    Color tooth  = (Color){ 240, 240, 220, 255 };
    Color dagger = (Color){ 200, 205, 215, 255 };
    ...
    DrawRectangle((int)torso.x, (int)(torso.y + torso.height - 6),
                  (int)torso.width, 6, skinLo);
...
void CombatantDrawWolf(Vector2 pos, bool facingRight, CombatantPose pose)
{
    ...
    Color fur    = (Color){ 130, 135, 145, 255 };
    Color furHi  = (Color){ 175, 180, 190, 255 };
    Color furLo  = (Color){  80,  85,  95, 255 };
    Color belly  = (Color){ 170, 170, 175, 255 };
    Color nose   = (Color){  30,  30,  35, 255 };
    Color eye    = (Color){ 230, 200,  70, 255 };
    Color claw   = (Color){  40,  40,  45, 255 };
    Color tongue = (Color){ 210,  90,  90, 255 };
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        DrawLineEx((Vector2){ lx, ly },
                   (Vector2){ lx + (facingRight ? 1.5f : -1.5f), ly + 18 },
                   6.0f, furLo);
    ...
    for (int i = 0; i < 6; i++)
    {
        float fx = px - 18.0f + i * 6.0f;
        DrawLineEx((Vector2){ fx, py - 12 }, (Vector2){ fx + 1, py - 4 }, 1.0f, furHi);
    }
...
void CombatantDrawOrc(Vector2 pos, bool facingRight, CombatantPose pose)
{
    ...
    Color skin    = (Color){  80, 140,  70, 255 };
    Color skinHi  = (Color){ 120, 180, 100, 255 };
    Color skinLo  = (Color){  50,  95,  45, 255 };
    Color horn    = (Color){ 220, 210, 190, 255 };
    Color hornLo  = (Color){ 160, 150, 130, 255 };
    Color armor   = (Color){ 110,  70,  40, 255 };
    Color armorLo = (Color){  70,  45,  25, 255 };
    Color metal   = (Color){ 130, 130, 135, 255 };
    Color eye     = (Color){ 220,  60,  40, 255 };
    Color tusk    = (Color){ 240, 235, 210, 255 };
